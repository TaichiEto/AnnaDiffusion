...
async def worker(process):
    ...
    while True:
        ...
        progress = 0
        try:
            old_file_path = get_latest_modified_file_path(
                OUT_DIR
            )  # 生成前の最新ファイルを取得
            process.stdin.write(
                (message.content[4:].replace("\n", " ") + "\n").encode()
            )  # プロンプトを送信
            await process.stdin.drain()

            await wait_for_output(
                process.stdout, "Type prompt:"
            )  # "Type prompt:" を待機
            latest_file_path = get_latest_modified_file_path(
                OUT_DIR
            )  # 生成後の最新ファイルを取得
            ...
            if latest_file_path == old_file_path:
                await message.channel.send(
                    (
                        f"{message.author.name}の{create_8char_hash(message.content)}じゃが…"
                        "どっか行っちゃったのじゃ。"
                    )
                )
            # 新しいファイルがある->正常（たぶん）
                ...
                with open(latest_file_path, "rb") as f:
                    pict = discord.File(f)
                    print(
                        f"生成ファイルを発表:{latest_file_path}, hash={create_8char_hash(message.content)}"
                    )
                    await message.channel.send(
                        f"{message.author.name}の"
...
async def on_request(ctx, *, message):
    if ctx.channel.name == CH_NAME:
        ...
        if not job_running and qsize == 0:
            oktext = (
                f"{ctx.author.name}、すぐ描いてやるぞ。"
                f"(hash:{create_8char_hash(ctx.message.content)})"
            )
        # ジョブが走っている
        elif qsize == 0:
            oktext = (
                f"{ctx.author.name}、次に描いてやるから待っておれよ。"
                f"(hash:{create_8char_hash(ctx.message.content)})"
            )
        # ジョブが走っている+キューが積まれている
        else:
            oktext = (
                f"{ctx.author.name}、{qsize+1}番目に描いてやるから待っておれよ。"
                f"(hash:{create_8char_hash(ctx.message.content)})"
            )
